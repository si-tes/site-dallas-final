...
export default function Login() {
  ...
  return (
    <div className="min-h-screen bg-[#fafafa] flex items-center justify-center p-4">
      ...
      >
        <h1 className="text-4xl font-black uppercase italic tracking-tighter mb-2">Entrar</h1>
        <p className="text-black/40 font-bold mb-8">Acesse sua conta para continuar.</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              ...
              <Link to="/esqueci-senha" className="text-[10px] font-bold text-black/40 hover:text-red-600 transition-colors uppercase italic">Esqueceu a senha?</Link>
            </div>
            <input 
              type="password" 
              required
        ...
        <p className="text-center font-bold text-black/40 mt-8 text-sm">
          Ainda não tem conta? <Link to={`/cadastro${searchParams.toString() ? `?${searchParams.toString()}` : ''}`} className="text-red-600 hover:underline">Cadastre-se</Link>
        </p>
      </motion.div>
    </div>
  );
