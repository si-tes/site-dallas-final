...
import { AlertCircle, ShoppingBag, ArrowRight, ArrowLeft } from 'lucide-react';
...
export default function Checkout() {
  ...
  // Carrega o perfil do usuário do backend se estiver logado
  useEffect(() => {
    if (token) {
      userService.getProfile(token).then(profile => {
        setFormData(prev => ({
          ...prev,
          usuario_id: profile.id || prev.usuario_id,
  ...
  const handlePayment = async () => {
    try {
      ...
      // Limpa o carrinho localmente após o sucesso do pedido
      carrinhoService.salvarCarrinho([]);
      // Dispara evento para o Header atualizar o contador de itens do carrinho
      ...
      <main className="pt-24 md:pt-28 pb-20 container mx-auto px-4 max-w-6xl">
        ...
        <div className="flex flex-col lg:flex-row gap-12 items-start">
            ...
            {step === 'details' && (
              <div className="space-y-8">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                      ...
                      <input name="nome" value={formData.nome} onChange={handleChange} type="text" className={`w-full bg-white border px-4 py-3 text-sm focus:outline-none focus:border-black transition-colors rounded-xl ${errors.nome ? 'border-red-500' : 'border-gray-300'}`} />
                    ...
                    <div>
                      ...
                      <input name="email" value={formData.email} onChange={handleChange} type="email" className={`w-full bg-white border px-4 py-3 text-sm focus:outline-none focus:border-black transition-colors rounded-xl ${errors.email ? 'border-red-500' : 'border-gray-300'}`} />
                    ...
                    <div>
                      ...
                      <input name="cpf" value={formData.cpf} onChange={handleChange} type="text" className={`w-full bg-white border px-4 py-3 text-sm focus:outline-none focus:border-black transition-colors rounded-xl ${errors.cpf ? 'border-red-500' : 'border-gray-300'}`} placeholder="Apenas números" />
                    ...
                    <div>
                      ...
                      <input name="telefone" value={formData.telefone} onChange={handleChange} type="text" className={`w-full bg-white border px-4 py-3 text-sm focus:outline-none focus:border-black transition-colors rounded-xl ${errors.telefone ? 'border-red-500' : 'border-gray-300'}`} placeholder="(00) 00000-0000" />
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                    <div>
                      ...
                      <input name="cep" value={formData.cep} onChange={handleChange} type="text" className={`w-full bg-white border px-4 py-3 text-sm focus:outline-none focus:border-black transition-colors rounded-xl ${errors.cep ? 'border-red-500' : 'border-gray-300'}`} placeholder="00000-000" />
                    ...
                    <div className="md:col-span-2">
                      ...
                      <input name="rua" value={formData.rua} onChange={handleChange} type="text" className={`w-full bg-white border px-4 py-3 text-sm focus:outline-none focus:border-black transition-colors rounded-xl ${errors.rua ? 'border-red-500' : 'border-gray-300'}`} />
                    ...
                    <div>
                      ...
                      <input name="numero" value={formData.numero} onChange={handleChange} type="text" className={`w-full bg-white border px-4 py-3 text-sm focus:outline-none focus:border-black transition-colors rounded-xl ${errors.numero ? 'border-red-500' : 'border-gray-300'}`} />
                    ...
                    <div className="md:col-span-2">
                      ...
                      <input name="complemento" value={formData.complemento} onChange={handleChange} type="text" className="w-full bg-white border border-gray-300 px-4 py-3 text-sm focus:outline-none focus:border-black transition-colors rounded-xl" placeholder="Apto, Bloco, etc" />
                    </div>
                    <div>
                      <label className="block text-[11px] font-black uppercase tracking-widest text-gray-500 mb-1">Bairro</label>
                      <input name="bairro" value={formData.bairro} onChange={handleChange} type="text" className={`w-full bg-white border px-4 py-3 text-sm focus:outline-none focus:border-black transition-colors rounded-xl ${errors.bairro ? 'border-red-500' : 'border-gray-300'}`} />
                    ...
                    <div>
                      ...
                      <input name="cidade" value={formData.cidade} onChange={handleChange} type="text" className={`w-full bg-white border px-4 py-3 text-sm focus:outline-none focus:border-black transition-colors rounded-xl ${errors.cidade ? 'border-red-500' : 'border-gray-300'}`} />
                    ...
                    <div>
                      ...
                      <input name="estado" value={formData.estado} onChange={handleChange} type="text" className={`w-full bg-white border px-4 py-3 text-sm focus:outline-none focus:border-black transition-colors rounded-xl ${errors.estado ? 'border-red-500' : 'border-gray-300'}`} placeholder="UF" />
                ...
                <button 
                  ...
                  className="w-full bg-black text-white py-5 rounded-full font-black uppercase italic tracking-wider text-sm hover:bg-red-600 transition-colors mt-8 shadow-lg hover:shadow-red-500/10"
                >
            ...
            {step === 'payment' && (
              <div className="space-y-8">
                <div>
                  ...
                  <div className="bg-[#f5f5f5] p-8 border border-gray-200 text-center rounded-2xl">
                    <img src="https://logospng.org/download/mercado-pago/logo-mercado-pago-icone-1024.png" alt="Mercado Pago" className="h-10 mx-auto mb-4 object-contain opacity-85" />
                    <h4 className="font-black uppercase tracking-tight text-base mb-2">Ambiente Seguro</h4>
                    <p className="text-sm text-gray-500 font-bold">
                      Você será redirecionado para o Mercado Pago para concluir o pagamento via PIX, Cartão de Crédito ou Boleto.
                    </p>
                  </div>
                </div>
                ...
                <button 
                  ...
                  className="w-full bg-black text-white py-5 rounded-full font-black uppercase italic tracking-wider text-sm hover:bg-red-600 transition-colors flex justify-center items-center gap-2 disabled:opacity-50 shadow-lg hover:shadow-red-500/10"
                >
          ...
          <div className="w-full lg:w-96 lg:sticky lg:top-24">
            <div className="bg-[#f5f5f5] p-6 border border-gray-200 rounded-2xl">
              ...
              <div className="space-y-4 max-h-[40vh] overflow-y-auto mb-6 pb-4 border-b border-gray-200">
                {itens.map((item) => (
                  <div key={item.id} className="flex gap-4">
                    <div className="w-16 h-20 bg-white flex-shrink-0 border border-gray-200 rounded-xl overflow-hidden">
                      <img 
                        src={item.imagem} 
                        alt={item.nome} 
              ...
              <div className="mb-6 pb-6 border-b border-gray-200">
                ...
                {cupomAplicado ? (
                  <div className="bg-green-50 border border-green-200 p-3 flex justify-between items-center rounded-xl">
                    <div>
                  ...
                  <div className="flex gap-2">
                    <input 
                      ...
                      className="bg-white border border-gray-300 px-3 py-2 text-xs font-bold uppercase flex-1 focus:outline-none focus:border-black transition-colors rounded-xl"
                    />
                    <button 
                      onClick={handleApplyCupom}
                      disabled={cupomLoading || !cupomCodigoInput.trim()}
                      className="bg-black text-white px-4 font-bold uppercase text-[10px] tracking-widest hover:bg-gray-800 transition-colors disabled:opacity-50 rounded-xl"
                    >
